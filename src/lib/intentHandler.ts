...
export class IntentRecognizer {
  ...
  private calculateConfidence(message: string, intentName: string): number {
    ...
    // 平台/资产匹配
    const platformOrAssetMatch = (pattern.platforms || pattern.assets || []).some(item =>
      normalizedMessage.includes(item.toLowerCase())
    );
    if (platformOrAssetMatch) score += pattern.weight.platform || pattern.weight.asset;

    // 上下文相关性判断
    if (intentName === 'supply' && normalizedMessage.includes('向') && normalizedMessage.includes('提供')) {
      score += 0.2;
    }
